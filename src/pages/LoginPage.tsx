...
// }
// pages/LoginPage.tsx
// After login/signup, if user wallet is not verified, show WalletSetupModal
import { useState } from 'react'
...
export default function LoginPage() {
  ...
  // Wallet setup modal state — shown after login if not verified
  const [showWalletSetup, setShowWalletSetup] = useState(false)

  function afterAuth() {
    // In production: check user.walletVerified from your auth response
    // Here we use the mock flag
    if (!MOCK_WALLET.verified) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    setLoading(true)
    setTimeout(() => { setLoading(false); afterAuth() }, 1400)
  }

  const handleGoogle = () => {
    setGoogleLoading(true)
    setTimeout(() => { setGoogleLoading(false); afterAuth() }, 1600)
  }
  ...
  return (
    <>
          ...
          <div className="flex-1 flex items-center justify-center px-6 py-12">
            <div className="w-full p-6 rounded-2xl max-w-[480px]">
              ...
              <div className="mb-8">
                ...
                <p className="text-sm" style={{ color: 'var(--color-text-muted)' }}>
                  {mode === 'login' ? 'Sign in to manage your tasks and earnings.' : 'Join 50,000+ people getting things done on Taskly.'}
                </p>
              </div>
              ...
              {mode === 'signup' && (
                <div className="mb-5">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {[{ value: 'poster', label: 'Post Tasks', sub: 'I need work done' }, { value: 'tasker', label: 'Be a Tasker', sub: 'I want to earn' }].map(({ value, label, sub }) => (
                      <button key={value} onClick={() => setRole(value as any)}
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    {mode === 'login' && <a href="#" className="text-xs" style={{ color: 'var(--color-accent)' }}>Forgot password?</a>}
                  </div>
                  <div className="relative">
                    <Lock size={16} className="absolute left-3.5 top-1/2 -translate-y-1/2" style={{ color: 'var(--color-text-muted)' }} />
                    <input type={showPass ? 'text' : 'password'} placeholder={mode === 'signup' ? 'Min. 8 characters' : '••••••••'} required
      ...
      {/* ── Wallet Setup Modal fires right after login ── */}
      {showWalletSetup && (
        <WalletSetupModal
          onComplete={() => { setShowWalletSetup(false); navigate('/wallet') }}
          onDismiss={() => { setShowWalletSetup(false); navigate('/dashboard') }}
        />
      )}
    </>
  )
